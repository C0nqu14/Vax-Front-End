...
export const CampaignDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        ...
        setError(err.response?.data?.error || "Falha ao carregar detalhes da campanha.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDonation = async () => {
    ...
    try {
      // O back-end esperado utiliza a tabela financiamento
  ...
  const handleReport = async (e: React.FormEvent) => {
    ...
    try {
      // Alinhado com a tabela SQL: denuncias (denunciante_id, usuario_denunciado_id, campanha_id, motivo)
  ...
  const percentage = Math.min((campanha.valor_arrecadado / campanha.valor_meta) * 100, 100);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          <div className="space-y-10">
             <div className="aspect-video rounded-[60px] overflow-hidden border border-vax-border shadow-3xl relative group">
                ...
                <div className="absolute top-10 left-10 flex gap-4">
                   <Badge className="bg-white/95 backdrop-blur shadow-2xl border-none font-black uppercase tracking-[0.2em] px-6 py-3 text-[10px] rounded-full">{campanha.categoria || "Social"}</Badge>
             ...
             <div className="space-y-6">
                <div className="flex flex-wrap items-center gap-4 text-xs font-black text-slate-400 uppercase tracking-widest px-2">
                   <MapPin className="w-4 h-4 text-vax-primary" /> Cazenga, Luanda • {new Date(campanha.data_criacao).toLocaleDateString()}
                </div>
          ...
          <div className="min-h-[400px]">
             <AnimatePresence mode="wait">
                ...
                {activeTab === 'apoiadores' && (
                  ...
                  >
                     {(campanha.financiamento || []).length > 0 ? (
                       campanha.financiamento.map((f: any, i: number) => (
                         <Card key={i} className="flex items-center gap-6 p-6 border-2 border-vax-border rounded-[32px] bg-white transition-all hover:border-vax-success-DEFAULT/20">
                            <Avatar name={f.usuarios?.nome_completo} src={f.usuarios?.avatar_url} size="md" />
                            <div className="flex-1 min-w-0">
                               <span className="block text-base font-bold text-vax-primary truncate tracking-tight">{f.usuarios?.nome_completo || "Doador Anónimo"}</span>
                       ...
                       <div className="col-span-2 text-center py-32 bg-vax-input/10 rounded-[60px] border-4 border-dashed border-vax-border flex flex-col items-center">
        ...
        <div className="space-y-10">
           <Card className="p-10 shadow-3xl bg-white border-vax-border rounded-[60px] relative overflow-hidden flex flex-col gap-10">
              ...
              <div className="space-y-8">
                 <div className="space-y-2">
                    ...
                    <span className="text-5xl font-bold text-vax-primary tracking-tighter tabular-nums leading-none">
                       {Number(campanha.valor_arrecadado).toLocaleString()} <span className="text-xl font-medium text-slate-300">KZ</span>
                    </span>
                 </div>

                 <div className="space-y-4">
                    <div className="flex justify-between text-[11px] font-black uppercase tracking-widest text-vax-primary">
                       <span>Meta: {Number(campanha.valor_meta).toLocaleString()} AKZ</span>
                 ...
                 <div className="grid grid-cols-2 gap-6">
                    <div className="p-6 bg-slate-50 rounded-[32px] border-2 border-vax-border text-center shadow-sm">
                       <h5 className="text-3xl font-bold text-vax-primary tabular-nums tracking-tighter leading-none mb-1">{(campanha.financiamento || []).length}</h5>
              ...
              <div className="space-y-8">
                 <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Valor da DOAÇÃO</label>
                    <div className="relative group">
                       <input 
                         type="number"
                         value={valorDoacao}
                         onChange={(e) => setValorDoacao(e.target.value)}
                         className="w-full bg-vax-input rounded-[30px] border-4 border-vax-border p-6 pr-16 font-black text-3xl text-vax-primary outline-none focus:border-vax-primary transition-all tabular-nums text-center shadow-inner group-hover:border-slate-300"
                       />
                 ...
                 <div className="space-y-4">
                    <Button 
                      ...
                      className={`w-full py-8 text-xl font-black shadow-2xl transition-all group relative overflow-hidden rounded-full ${
                        user && !user.ativo 
                    ...
                    >
                       <span className="relative z-10 flex items-center justify-center gap-4">
                          {user && !user.ativo ? "NIF pendente AGT" : "Apoiar agora"} <Zap className={`w-6 h-6 transition-transform ${user && !user.ativo ? 'opacity-20' : 'group-hover:scale-110 group-hover:rotate-12'}`} />
                       </span>
                    </Button>
           ...
           <div className="flex flex-col gap-6 pt-4">
              <Card className="p-8 transition-all hover:bg-slate-50 cursor-pointer group rounded-[40px] border-vax-border shadow-vax" onClick={() => navigate(`/perfil/${campanha.usuario_id}`)}>
                 <div className="flex items-center gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                       <h4 className="text-base font-bold text-vax-primary group-hover:underline truncate tracking-tight">Criado por {campanha.usuarios?.nome_completo || "Indivíduo Vax"}</h4>
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1 flex items-center gap-3">
                          <Badge className="bg-vax-primary/10 text-vax-primary p-0 border-none">LVL {campanha.usuarios?.level || 1}</Badge>
                          • {campanha.usuarios?.n_campanhas || 0} Iniciativas
                       </p>
                    </div>
